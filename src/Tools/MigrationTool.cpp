...
int main(int argc, char *argv[])
{
  ...
  command_line::add_arg(options, arg_size_limit);
  command_line::add_arg(options, arg_no_limit);
  command_line::add_arg(options, arg_bulk);
  command_line::add_arg(options, arg_rescue);
  ...
  if (vm.count("help"))
  {
    std::cout << "Conceal Migration Tool\n\n"
              << "Usage example:\n"
              << "  ./conceal-migrate --old-dir ~/.conceal --new-dir ~/.conceal-mdbx\n\n"
              << required << "\n"
              << options << std::endl;
    return 0;
  }
