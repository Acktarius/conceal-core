// Conceal CN-GPU mining kernels (xmr-stak cryptonight_gpu.cl adapted,
// under Licence GPL-3.0, https://www.gnu.org/licenses/gpl-3.0.html).
// Build with: -DWORKSIZE=N -DMEMORY=2097152 -DMASK=2097088 -DITERATIONS=49152
...
#endif
#ifndef MEMORY
#define MEMORY (2 * 1024 * 1024)
#endif
#ifndef MASK
#define MASK (((MEMORY - 1) >> 6) << 6)
#endif
#ifndef ITERATIONS
#define ITERATIONS 49152
#endif
#ifndef COMP_MODE
#define COMP_MODE 1
#endif
...
inline int4 _mm_alignr_epi8(int4 a, const uint rot)
{
  ...
  return (int4)(((uint)a.s0 >> right) | (a.s1 << left), ((uint)a.s1 >> right) | (a.s2 << left),
                ((uint)a.s2 >> right) | (a.s3 << left), ((uint)a.s3 >> right) | (a.s0 << left));
}

inline global int4* scratchpad_ptr(uint idx, uint n, __global int* lpad)
{
...
inline int4 single_compute(float4 n0, float4 n1, float4 n2, float4 n3, float cnt, float4 rnd_c,
                           __local float4* sum)
{
  float4 c = (float4)(cnt);
...
inline void single_compute_wrap(const uint rot, int4 v0, int4 v1, int4 v2, int4 v3, float cnt,
                                float4 rnd_c, __local float4* sum, __local int4* out)
{
...
static const __constant uint look[16][4] = {
    {0, 1, 2, 3}, {0, 2, 3, 1}, {0, 3, 1, 2}, {0, 3, 2, 1}, {1, 0, 2, 3}, {1, 2, 3, 0},
    {1, 3, 0, 2}, {1, 3, 2, 0}, {2, 1, 0, 3}, {2, 0, 3, 1}, {2, 3, 1, 0}, {2, 3, 0, 1},
    {3, 1, 2, 0}, {3, 2, 0, 1}, {3, 0, 1, 2}, {3, 0, 2, 1}};

static const __constant float ccnt[16] = {
    1.34375f,   1.28125f,   1.359375f,  1.3671875f, 1.4296875f, 1.3984375f, 1.3828125f,
    1.3046875f, 1.4140625f, 1.2734375f, 1.2578125f, 1.2890625f, 1.3203125f, 1.3515625f,
    1.3359375f, 1.4609375f};
...
static const __constant uint skip[3] = {20, 22, 22};
