...
namespace cn
{
  namespace
  {
    ...
    const command_line::arg_descriptor<size_t> arg_p2p_min_connections = {"p2p-min-connections", "Minimum outgoing connections", P2P_DEFAULT_MIN_CONNECTIONS};
    const command_line::arg_descriptor<size_t> arg_p2p_max_connections = {"p2p-max-connections", "Maximum outgoing connections", P2P_DEFAULT_MAX_CONNECTIONS};
  ...
  void NetNodeConfig::initOptions(boost::program_options::options_description &desc)
  {
    ...
    command_line::add_arg(desc, arg_p2p_min_connections);
    command_line::add_arg(desc, arg_p2p_max_connections);
  }
  ...
  NetNodeConfig::NetNodeConfig()
  {
    ...
    testnet = false;
    minOutgoingConnections = cn::P2P_DEFAULT_MIN_CONNECTIONS;
    maxOutgoingConnections = cn::P2P_DEFAULT_MAX_CONNECTIONS;
  }
  ...
  bool NetNodeConfig::init(const boost::program_options::variables_map &vm)
  {
    ...
    if (vm.count(arg_p2p_min_connections.name) != 0)
      minOutgoingConnections = command_line::get_arg(vm, arg_p2p_min_connections);

    if (vm.count(arg_p2p_max_connections.name) != 0)
      maxOutgoingConnections = command_line::get_arg(vm, arg_p2p_max_connections);

    if (maxOutgoingConnections < minOutgoingConnections)
      maxOutgoingConnections = minOutgoingConnections;
  ...
  size_t NetNodeConfig::getMinOutgoingConnections() const
  {
    return minOutgoingConnections;
  }

  size_t NetNodeConfig::getMaxOutgoingConnections() const
  {
  ...
  void NetNodeConfig::setMinOutgoingConnections(size_t count)
  {
    minOutgoingConnections = count;
  }

  void NetNodeConfig::setMaxOutgoingConnections(size_t count)
  {
    maxOutgoingConnections = std::max(count, minOutgoingConnections);
  }
